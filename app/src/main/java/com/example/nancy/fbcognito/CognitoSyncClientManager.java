...
public class CognitoSyncClientManager {
    ...
    private static final String IDENTITY_POOL_ID = "eu-west-2:92cd49ed-8caf-4079-8589-24556878a20b";
    ...
    public static void init(Context context) {
        ...
        if (syncClient != null) return;
            ...
            credentialsProvider = new CognitoCachingCredentialsProvider(context, IDENTITY_POOL_ID,
                    REGION);
            System.out.println("Nancy Developer authenticated identities is not configured");
    ...
    public static void addLogins(String providerName, String token) {
        System.out.println("Nancy  privider name "+providerName+" token "+token);
        if (syncClient == null) {
            System.out.println("Nancy  syncClient null");
            throw new IllegalStateException("CognitoSyncClientManager not initialized yet");
        }
        else{
            System.out.println("Nancy  syncClient not null");
        }


        Map<String, String> logins = credentialsProvider.getLogins();

        System.out.println("Nancy "+credentialsProvider.getLogins());

        if (logins == null) {
            System.out.println("Nancy login null");
